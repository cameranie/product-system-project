...
export function RequirementPoolPageWithReviewers({ context, onNavigate }: RequirementPoolPageProps = {}) {
  const [requirements, setRequirements] = useState<Requirement[]>(mockRequirements);
  const [searchTerm, setSearchTerm] = useState('');
  const [hiddenColumns, setHiddenColumns] = useState<string[]>(['reviewer1', 'reviewer2', 'reviewStatus', 'scheduledVersion']);
  ...
  // 批量添加版本号
  const handleBatchAddVersion = (version: string) => {
    const eligibleRequirements = requirements.filter(r => 
      selectedRequirements.includes(r.id) && 
    ...
    if (eligibleRequirements.length === 0) {
      toast.error('所选需求中没有可以添加版本号的需求（需要评审通过）');
      return;
    }

    const updatedRequirements = requirements.map(r => {
      if (selectedRequirements.includes(r.id) && getReviewStatus(r) === '评审通过') {
    ...
    setRequirements(updatedRequirements);
    setSelectedRequirements([]);
    toast.success(`已为 ${eligibleRequirements.length} 个需求批量设置版本号: ${version}`);
  };
    ...
    return filters.every(filter => {
      ...
      switch (filter.column) {
        ...
        case 'reviewStatus':
          ...
          break;
        case 'reviewer1':
          fieldValue = requirement.reviewer1 ? 
            `${requirement.reviewer1.name} ${requirement.reviewer1Status ? reviewerStatusLabels[requirement.reviewer1Status].label : ''}` : '';
          break;
        case 'reviewer2':
          fieldValue = requirement.reviewer2 ? 
            `${requirement.reviewer2.name} ${requirement.reviewer2Status ? reviewerStatusLabels[requirement.reviewer2Status].label : ''}` : '';
          break;
  ...
  // 处理预排期版本号修改
  const handleScheduledVersionChange = (requirementId: string, version: string) => {
    const updatedRequirements = requirements.map(r => 
      r.id === requirementId 
        ? { ...r, scheduledVersion: version }
        : r
    );
    setRequirements(updatedRequirements);
    toast.success('预排期版本号已更新');
  };
                ...
                <div className="flex items-center gap-2">
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      <DropdownMenuItem onClick={handleBatchApproveFirstLevel}>
                        <Target className="h-4 w-4 mr-2" />
                        批量通过一级评审
                      </DropdownMenuItem>
                      <DropdownMenuItem onClick={handleBatchApproveSecondLevel}>
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      {versionOptions.map((version) => (
                        <DropdownMenuItem 
                          key={version} 
                          onClick={() => handleBatchAddVersion(version)}
                        >
                          {version}
                        </DropdownMenuItem>
                      ))}
              ...
              <TableBody>
                {sortedRequirements.map((requirement) => (
                  <TableRow key={requirement.id}>
                    ...
                    <TableCell>
                      <Select
                        value={requirement.needToDo}
                        onValueChange={(value) => handleNeedToDoChange(requirement.id, value as '是' | '否')}
                      >
                      ...
                      <TableCell>
                        {requirement.reviewer2 ? (
                          <div className="space-y-1">
                            {/* 二级评审员姓名 */}
                            <div className="text-sm">{requirement.reviewer2.name}</div>
                            {/* 二级评审状态选择 */}
                            <Select
                      ...
                      <TableCell>
                        {canSetVersion(requirement) ? (
                          <Select
                            ...
                            onValueChange={(value) => handleScheduledVersionChange(requirement.id, value)}
                          >
                            <SelectTrigger className="h-8 text-sm">
                          ...
                          <Badge variant="outline">{requirement.scheduledVersion}</Badge>
                        ) : (
                          <span className="text-muted-foreground text-sm">
                            {getReviewStatus(requirement) === '评审通过' ? '未指定' : '待评审通过'}
                          </span>
                        )}
                      </TableCell>
                    )}
