...
const mockRequirements: Requirement[] = [
  ...
  },
  {
    ...
    description: '升级行情��送服务架构，支持更高并发量的实时行情数据推送，降低延迟。',
...
const filterableColumns = [
  ...
  { value: 'reviewer1', label: '一级评审人员' },
  { value: 'reviewer1Status', label: '一级评审状态' },
  { value: 'reviewer2', label: '二级评审人员' },
...
export function RequirementPoolPageWithHeaderSort({ context, onNavigate }: RequirementPoolPageProps = {}) {
    ...
    return filters.every(filter => {
      ...
      switch (filter.column) {
        ...
        case 'reviewStatus':
          ...
          break;
        case 'reviewer1':
          fieldValue = requirement.reviewer1 ? requirement.reviewer1.name : '';
          break;
        case 'reviewer1Status':
          fieldValue = requirement.reviewer1Status ? reviewerStatusLabels[requirement.reviewer1Status].label : '';
          break;
        case 'reviewer2':
          fieldValue = requirement.reviewer2 ? requirement.reviewer2.name : '';
          break;
  ...
  const handleBatchSetVersion = (version: string) => {
    ...
    if (version === 'clear') {
      ...
      setRequirements(updatedRequirements);
      setSelectedRequirements([]);
      toast.success(`已清除 ${selectedRequirements.length} 个需求的���本号`);
      return;
    }
    ...
    setRequirements(updatedRequirements);
    setSelectedRequirements([]);
    toast.success(`已为 ${eligibleRequirements.length} 个需求设置���本号: ${version}`);
  };
  ...
  // 检查是否可以设置版本号（需要两级评审都通过）
  const canSetVersion = (requirement: Requirement): boolean => {
    // 如果只有一级评审人员，一级评审通过即可
    if (requirement.reviewer1 && !requirement.reviewer2) {
      return requirement.reviewer1Status === 'approved';
    }
    // 如果有两级评审人员，都必须通过
    if (requirement.reviewer1 && requirement.reviewer2) {
      return requirement.reviewer1Status === 'approved' && requirement.reviewer2Status === 'approved';
    }
    return false;
  };
  ...
  const handleVersionChange = (requirementId: string, version: string) => {
    ...
    if (!requirement || (!canSetVersion(requirement) && version !== 'clear')) {
    ...
    const updatedRequirements = requirements.map(r => 
      r.id === requirementId 
        ? { ...r, scheduledVersion: version === 'clear' ? undefined : version }
        : r
    );
    setRequirements(updatedRequirements);
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <Button variant="ghost" size="sm" onClick={() => setSelectedRequirements([])}>
                    <X className="h-4 w-4" />
                  </Button>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      <DropdownMenuItem onClick={() => handleBatchSetVersion('clear')}>
                        清除版本号
                      </DropdownMenuItem>
                      <DropdownMenuSeparator />
                      {versionOptions.map(version => (
                        <DropdownMenuItem 
                          key={version}
                          onClick={() => handleBatchSetVersion(version)}
                        >
                          {version}
                        </DropdownMenuItem>
                      ))}
                ...
                <TableHeader>
                  <TableRow>
                    <TableHead className="w-12">
                      <Checkbox 
                        checked={sortedRequirements.length > 0 && selectedRequirements.length === sortedRequirements.length}
                    ...
                    <TableHead>
                      <Button
                        variant="ghost"
                        className="h-auto p-0 hover:bg-transparent flex items-center text-left justify-start font-medium"
                    ...
                    {!hiddenColumns.includes('reviewStatus') && (
                      <TableHead>
                        <Button
                          variant="ghost"
                          className="h-auto p-0 hover:bg-transparent flex items-center text-left justify-start font-medium"
                          onClick={() => handleColumnSort('reviewStatus')}
                        >
                          总评审状态
                    ...
                    {!hiddenColumns.includes('reviewer1Status') && (
                      <TableHead>
                        <Button
                          variant="ghost"
                          className="h-auto p-0 hover:bg-transparent flex items-center text-left justify-start font-medium"
                          onClick={() => handleColumnSort('reviewer1Status')}
                        >
                          一级评审状态
                          {getSortIcon('reviewer1Status')}
                        </Button>
                      </TableHead>
                    )}
                    {!hiddenColumns.includes('reviewer2Status') && (
                      <TableHead>
                        <Button
                          variant="ghost"
                          className="h-auto p-0 hover:bg-transparent flex items-center text-left justify-start font-medium"
                          onClick={() => handleColumnSort('reviewer2Status')}
                        >
                          二级评审状态
                          {getSortIcon('reviewer2Status')}
                        </Button>
                      </TableHead>
                    )}
                    {!hiddenColumns.includes('scheduledVersion') && (
                      <TableHead>
                        <Button
                          variant="ghost"
                          className="h-auto p-0 hover:bg-transparent flex items-center text-left justify-start font-medium"
                          onClick={() => handleColumnSort('scheduledVersion')}
                        >
                          预排期版本号
                          {getSortIcon('scheduledVersion')}
                        </Button>
                      </TableHead>
                    )}
                  </TableRow>
                    ...
                    <TableRow key={requirement.id} className="cursor-pointer hover:bg-muted/50">
                      ...
                      {!hiddenColumns.includes('reviewStatus') && (
                        <TableCell onClick={() => handleViewRequirement(requirement)}>
                          <Badge 
                            variant="secondary"
                            className={getReviewStatusStyle(getReviewStatus(requirement))}
                          >
                      ...
                      {!hiddenColumns.includes('scheduledVersion') && (
                        <TableCell>
                          {canSetVersion(requirement) ? (
                            <Select
                              value={requirement.scheduledVersion || 'none'}
                              onValueChange={(value) => handleVersionChange(requirement.id, value === 'none' ? 'clear' : value)}
                            >
                              <SelectTrigger className="h-7 text-xs">
                                <SelectValue />
                              </SelectTrigger>
                            ...
                            <span className="text-xs text-muted-foreground">{getVersionDisplay(requirement)}</span>
                          )}
                        </TableCell>
                      )}
                    </TableRow>
