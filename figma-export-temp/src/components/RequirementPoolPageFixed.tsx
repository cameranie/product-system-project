...
export function RequirementPoolPage({ context, onNavigate }: RequirementPoolPageProps = {}) {
  ...
  // 检查选中需求的一级评审状态
  const getFirstLevelEligibleCount = () => {
  ...
  // 检查选中需求的二级评审状态
  const getSecondLevelEligibleCount = () => {
    return requirements.filter(r => 
      selectedRequirements.includes(r.id) && 
      r.reviewer2 && 
      r.reviewer1Status === 'approved' && 
  ...
  // 检查是否可以设置版本号（需要评审通过）
  const canSetVersion = (requirement: Requirement): boolean => {
    return getReviewStatus(requirement) === '评审通过';
  };

  // 获取显示的版本号内容
  const getVersionDisplay = (requirement: Requirement): string => {
    return canSetVersion(requirement) 
  ...
  // 批量通过一级评审
  const handleBatchApproveFirstLevel = () => {
    ...
    const updatedRequirements = requirements.map(r => {
      if (selectedRequirements.includes(r.id) && r.reviewer1 && r.reviewer1Status !== 'approved') {
        return { ...r, reviewer1Status: 'approved' };
      }
      return r;
    });
  ...
  // 批量通过二级评审
  const handleBatchApproveSecondLevel = () => {
    const eligibleRequirements = requirements.filter(r => 
      selectedRequirements.includes(r.id) && 
      r.reviewer2 && 
      r.reviewer1Status === 'approved' && 
    ...
    const updatedRequirements = requirements.map(r => {
      if (selectedRequirements.includes(r.id) && 
          ...
          r.reviewer1Status === 'approved' && 
          r.reviewer2Status !== 'approved') {
        return { ...r, reviewer2Status: 'approved' };
      }
      return r;
    });
    setRequirements(updatedRequirements);
  ...
  // 批量添加版本号
  const handleBatchSetVersion = (version: string) => {
    // 如果是清除版本，允许对所有选中的需求操作
    if (version === '') {
      const updatedRequirements = requirements.map(r => {
        if (selectedRequirements.includes(r.id)) {
          return { ...r, scheduledVersion: undefined };
        }
        return r;
      });
      setRequirements(updatedRequirements);
      setSelectedRequirements([]);
      toast.success(`已清除 ${selectedRequirements.length} 个需求的版本号`);
      return;
    }

    // 设置版本号需要评审通过
    const eligibleRequirements = requirements.filter(r => 
      selectedRequirements.includes(r.id) && canSetVersion(r)
    );
    
    if (eligibleRequirements.length === 0) {
      toast.error('所选需求中没有可以设置版本号的需求（需要先通过评审）');
      return;
    }

    const updatedRequirements = requirements.map(r => {
      if (selectedRequirements.includes(r.id) && canSetVersion(r)) {
        return { ...r, scheduledVersion: version };
      }
      return r;
    });
    setRequirements(updatedRequirements);
    setSelectedRequirements([]);
    toast.success(`已为 ${eligibleRequirements.length} 个需求设置版本号: ${version}`);
  };
  ...
  const handleVersionChange = (requirementId: string, version: string) => {
    ...
    if (!requirement || (!canSetVersion(requirement) && version !== '')) {
      toast.error('需要先通过评审才能设置版本号');
      return;
    }

    const updatedRequirements = requirements.map(r => 
      r.id === requirementId 
        ? { ...r, scheduledVersion: version || undefined }
        : r
    );
    setRequirements(updatedRequirements);
                ...
                <div className="flex items-center gap-2">
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="start">
                      ...
                      <DropdownMenuItem onClick={() => toggleColumnVisibility('scheduledVersion')}>
                        <Checkbox className="mr-2" checked={!hiddenColumns.includes('scheduledVersion')} />
                        预排期版本号
                      </DropdownMenuItem>
                    </DropdownMenuContent>
                  </DropdownMenu>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <Button 
                    ...
                    disabled={getFirstLevelEligibleCount() === 0}
                  >
                    <CheckCircle className="h-4 w-4 mr-2" />
                    批量通过一级评审
                    {getFirstLevelEligibleCount() > 0 && (
                      <span className="ml-1 text-xs">({getFirstLevelEligibleCount()})</span>
                    )}
                  </Button>
                  <Button 
                    variant="outline" 
                    ...
                    disabled={getSecondLevelEligibleCount() === 0}
                  >
                    <CheckSquare className="h-4 w-4 mr-2" />
                    批量通过二级评审
                    {getSecondLevelEligibleCount() > 0 && (
                      <span className="ml-1 text-xs">({getSecondLevelEligibleCount()})</span>
                    )}
                  </Button>
                  <Select onValueChange={(value) => handleBatchSetVersion(value === 'none' ? '' : value)}>
                    <SelectTrigger className="w-40">
                      <SelectValue placeholder="批量设置版本号" />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="none">清除版本</SelectItem>
              ...
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="w-12 text-center">
                    <Button
                      ...
                      onClick={() => handleSort('title', 
                        sortConfig?.column === 'title' && sortConfig?.direction === 'asc' ? 'desc' : 'asc'
                      )}
                      className="h-8 p-0"
                    >
                      #
                    ...
                    <TableHead>
                      <Button
                        ...
                        onClick={() => handleSort('type', 
                          sortConfig?.column === 'type' && sortConfig?.direction === 'asc' ? 'desc' : 'asc'
                        )}
                        className="h-8 p-0"
                      >
                        需求类型
                    ...
                    <TableHead>
                      <Button
                        ...
                        onClick={() => handleSort('priority', 
                          sortConfig?.column === 'priority' && sortConfig?.direction === 'asc' ? 'desc' : 'asc'
                        )}
                        className="h-8 p-0"
                      >
                        优先级
                    ...
                    <TableHead>
                      <Button
                        ...
                        onClick={() => handleSort('creator', 
                          sortConfig?.column === 'creator' && sortConfig?.direction === 'asc' ? 'desc' : 'asc'
                        )}
                        className="h-8 p-0"
                      >
                        创建人
                    ...
                    <TableHead>
                      <Button
                        ...
                        onClick={() => handleSort('reviewStatus', 
                          sortConfig?.column === 'reviewStatus' && sortConfig?.direction === 'asc' ? 'desc' : 'asc'
                        )}
                        className="h-8 p-0"
                      >
                        总评审状态
                        {sortConfig?.column === 'reviewStatus' && (
                          sortConfig.direction === 'asc' ? 
                          <ArrowUp className="ml-1 h-3 w-3" /> : 
                          <ArrowDown className="ml-1 h-3 w-3" />
                        )}
                  ...
                  {!hiddenColumns.includes('reviewer1Status') && (
                    <TableHead>一级评审状态</TableHead>
                  )}
                  
                  {/* 二级评审状态列 */}
                  {!hiddenColumns.includes('reviewer2Status') && (
                    <TableHead>二级评审状态</TableHead>
                  )}
                  ...
                  {!hiddenColumns.includes('scheduledVersion') && (
                    <TableHead>
                      <Button
                        variant="ghost"
                        size="sm"
                        onClick={() => handleSort('scheduledVersion', 
                          sortConfig?.column === 'scheduledVersion' && sortConfig?.direction === 'asc' ? 'desc' : 'asc'
                        )}
                        className="h-8 p-0"
                      >
                        预排期版本号
                        {sortConfig?.column === 'scheduledVersion' && (
                          sortConfig.direction === 'asc' ? 
                          <ArrowUp className="ml-1 h-3 w-3" /> : 
                          <ArrowDown className="ml-1 h-3 w-3" />
                        )}
              ...
              <TableBody>
                {sortedRequirements.map((requirement, index) => (
                  <TableRow key={requirement.id}>
                    ...
                    <TableCell>
                      <div className="cursor-pointer hover:text-primary" onClick={() => handleViewRequirement(requirement)}>
                        {requirement.title}
                      </div>
                    </TableCell>
                    ...
                    {!hiddenColumns.includes('priority') && (
                      <TableCell>
                        <Badge 
                          ...
                          className={`text-xs ${priorityConfig[requirement.priority].className}`}
                        >
                    ...
                    {!hiddenColumns.includes('reviewStatus') && (
                      <TableCell>
                        <Badge 
                          variant="outline" 
                          className={`text-xs ${getReviewStatusStyle(getReviewStatus(requirement))}`}
                        >
                        ...
                        {requirement.reviewer1 ? (
                          <div className="space-y-1">
                            <div className="flex items-center gap-1 text-xs text-muted-foreground">
                              <span>{requirement.reviewer1.name}</span>
                            </div>
                            <Select
                              value={requirement.reviewer1Status || 'pending'}
                              onValueChange={(value) => handleReviewer1StatusChange(requirement.id, value as 'pending' | 'approved' | 'rejected')}
                            >
                    ...
                    {!hiddenColumns.includes('reviewer2Status') && (
                      <TableCell>
                        {requirement.reviewer2 ? (
                          <div className="space-y-1">
                            <div className="flex items-center gap-1 text-xs text-muted-foreground">
                              <span>{requirement.reviewer2.name}</span>
                            </div>
                            <Select
                              value={requirement.reviewer2Status || 'pending'}
                              onValueChange={(value) => handleReviewer2StatusChange(requirement.id, value as 'pending' | 'approved' | 'rejected')}
                    ...
                    {!hiddenColumns.includes('scheduledVersion') && (
                      <TableCell>
                        {canSetVersion(requirement) ? (
                          <Select
                            value={requirement.scheduledVersion || 'empty'}
                            onValueChange={(value) => handleVersionChange(requirement.id, value === 'empty' ? '' : value)}
                          >
          ...
          <Card>
            <CardContent className="p-8 text-center">
              <div className="text-muted-foreground">
                ...
                <p>暂无需求数据</p>
                <p className="text-sm mt-1">您可以创建新的产品需求</p>
              </div>
            </CardContent>
          </Card>
        )}
