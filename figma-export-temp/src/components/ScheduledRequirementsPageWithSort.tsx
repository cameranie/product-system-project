...
export function ScheduledRequirementsPageWithSort({ context, onNavigate }: ScheduledRequirementsPageProps = {}) {
      ...
      <div className="flex-1 overflow-auto p-6 space-y-6">
                    ...
                    <DropdownMenuContent align="start">
                      {sortableColumns.map((col) => (
                        <React.Fragment key={col.value}>
                          <DropdownMenuItem onClick={() => setSortConfig({ column: col.value, direction: 'asc' })}>
                            <ArrowUp className="h-3 w-3 mr-2" />
                            {col.label} ↑
                          </DropdownMenuItem>
                          <DropdownMenuItem onClick={() => setSortConfig({ column: col.value, direction: 'desc' })}>
          ...
          <Card className="border-l-4 border-l-primary">
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <Button size="sm" variant="outline" onClick={() => handleBatchReview('1')}>
                    <CheckSquare className="h-4 w-4 mr-2" />
                    批量通过一级评审
                  </Button>
                  <Button size="sm" variant="outline" onClick={() => handleBatchReview('2')}>
                    <CheckCircle className="h-4 w-4 mr-2" />
                    批量通过二级评审
                  </Button>
                  <Button size="sm" variant="default" onClick={handleBatchMoveToVersionRequirements}>
        ...
        <div className="space-y-6">
          {sortedVersions.map((version) => {
            ...
            const completedCount = versionRequirements.filter(r => r.status === '已完成').length;
            const totalCount = versionRequirements.length;
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <span className="text-sm text-muted-foreground">
                              {totalCount} 个需求 · {completedCount} 个已完成
                            </span>
                          </div>
                        </div>
                        <div className="text-sm text-muted-foreground">
                          完成度: {totalCount > 0 ? Math.round((completedCount / totalCount) * 100) : 0}%
                        </div>
                      </div>
                    </CardContent>
                  </CollapsibleTrigger>
                        ...
                        <TableHeader>
                          <TableRow>
                            <TableHead className="w-10">
                              <Checkbox
                                checked={versionRequirements.length > 0 && versionRequirements.every(req => selectedRequirements.includes(req.id))}
                                onCheckedChange={(checked) => {
                                  if (checked) {
                                    setSelectedRequirements([...selectedRequirements, ...versionRequirements.filter(req => !selectedRequirements.includes(req.id)).map(req => req.id)]);
                                  } else {
                                    setSelectedRequirements(selectedRequirements.filter(id => !versionRequirements.map(req => req.id).includes(id)));
                                  }
                                }}
                              />
                            </TableHead>
                            {!hiddenColumns.includes('title') && (
                              <TableHead className="cursor-pointer hover:bg-muted/50" onClick={() => handleColumnSort('title')}>
                            ...
                            {!hiddenColumns.includes('type') && (
                              <TableHead className="cursor-pointer hover:bg-muted/50" onClick={() => handleColumnSort('type')}>
                                <div className="flex items-center">
                                  需求类型
                                  {getSortIcon('type')}
                                </div>
                            ...
                            {!hiddenColumns.includes('priority') && (
                              <TableHead className="cursor-pointer hover:bg-muted/50" onClick={() => handleColumnSort('priority')}>
                                <div className="flex items-center">
                                  优先级
                            ...
                            {!hiddenColumns.includes('platform') && (
                              <TableHead className="cursor-pointer hover:bg-muted/50" onClick={() => handleColumnSort('platform')}>
                                <div className="flex items-center">
                                  应用端
                            ...
                            {!hiddenColumns.includes('creator') && (
                              <TableHead className="cursor-pointer hover:bg-muted/50" onClick={() => handleColumnSort('creator')}>
                                <div className="flex items-center">
                                  创建人
                                  {getSortIcon('creator')}
                                </div>
                            ...
                            {!hiddenColumns.includes('reviewStatus') && (
                              <TableHead className="cursor-pointer hover:bg-muted/50" onClick={() => handleColumnSort('reviewStatus')}>
                                <div className="flex items-center">
                                  总评审状态
                            ...
                            {!hiddenColumns.includes('reviewer1Status') && (
                              <TableHead className="cursor-pointer hover:bg-muted/50" onClick={() => handleColumnSort('reviewer1')}>
                                <div className="flex items-center">
                                  一级评审
                                  {getSortIcon('reviewer1')}
                                </div>
                              </TableHead>
                            )}
                            {!hiddenColumns.includes('reviewer2Status') && (
                              <TableHead className="cursor-pointer hover:bg-muted/50" onClick={() => handleColumnSort('reviewer2')}>
                                <div className="flex items-center">
                                  二级评审
                                  {getSortIcon('reviewer2')}
                                </div>
                              </TableHead>
                            )}
                            {!hiddenColumns.includes('plannedVersion') && (
                              <TableHead className="cursor-pointer hover:bg-muted/50" onClick={() => handleColumnSort('plannedVersion')}>
                                <div className="flex items-center">
                                  预排期版本号
                                  {getSortIcon('plannedVersion')}
                                </div>
                              </TableHead>
                            )}
                          </TableRow>
                                ...
                                <TableCell>
                                  {requirement.reviewer1 ? (
                                    <div className="space-y-1">
                                      ...
                                      <Select
                                        value={requirement.reviewer1Status}
                                ...
                                <TableCell>
                                  {requirement.reviewer2 ? (
                                    <div className="space-y-1">
                                      ...
                                      <Select
                                        value={requirement.reviewer2Status}
                                        onValueChange={(value: 'pending' | 'approved' | 'rejected') => 
                              ...
                              {!hiddenColumns.includes('plannedVersion') && (
                                <TableCell>
                                  ...
                                  >
                                    ...
                                    <SelectContent>
                                      <SelectItem value="none">清除版本号</SelectItem>
                                      <DropdownMenuSeparator />
            ...
            <Card>
              <CardContent className="p-8">
                <div className="text-center text-muted-foreground">
                  ...
                  <h3 className="font-medium mb-2">暂无预排期需求</h3>
                  <p className="text-sm">
                    所有需求都还在需求池中，请先从需求池添加预排期版本号。
                  </p>
                </div>
              </CardContent>
            </Card>
