...
export function VersionRequirementsPageWithLayoutUpdated({ onNavigate }: VersionRequirementsPageProps) {
    ...
    return filters.every(filter => {
      ...
      switch (filter.column) {
        ...
        case 'platform':
          ...
          break;
        case 'assignee':
          fieldValue = requirement.assignee.name;
          break;
        case 'creator':
                        ...
                        {groupedByVersion[version].map((requirement) => (
                          <React.Fragment key={requirement.id}>
                            ...
                            <TableRow className="hover:bg-gray-50">
                              ...
                              {!hiddenColumns.includes('type') && (
                                <TableCell>
                                  <Badge variant="secondary" className={getTypeColor(requirement.type)}>
                                    {requirement.type}
                                  </Badge>
                                </TableCell>
                              )}
                              ...
                              <TableCell>
                                <DropdownMenu>
                                  ...
                                  <DropdownMenuContent align="end">
                                    <DropdownMenuItem 
                                      onSelect={() => handleRequirementTitleClick(requirement)}
                                    >
                                      查看详情
                                    </DropdownMenuItem>
                                    <DropdownMenuItem>
                                      ...
                                      {requirement.subtasks.map((subtask) => (
                                        ...
                                        >
                                          <div className="flex items-center gap-3 flex-1">
                                            ...
                                            <Badge 
                                              variant="secondary" 
                                              className={getSubtaskStatusColor(subtask.status)}
                                            >
                                              {subtask.status}
                                            </Badge>
                                            <Badge 
                                          ...
                                          <div className="flex items-center gap-6 text-sm text-gray-600">
                                            {subtask.executor && (
                                              <div className="flex items-center gap-2">
                                                ...
                                                <span>{subtask.executor.name}</span>
                                              </div>
                                            )}
                                            {subtask.estimatedStartDate && (
                                              <div className="flex items-center gap-2">
                                                <Clock className="h-3 w-3" />
                                                <span>{formatDateTime(subtask.estimatedStartDate)}</span>
                                              </div>
                                            )}
