...
export function RequirementPoolPageWithSorting({ context, onNavigate }: RequirementPoolPageProps = {}) {
  ...
  const handleBatchApproveFirstLevel = () => {
    ...
    if (eligibleRequirements.length === 0) {
      toast.error('所选需求中没有可以通���一级评审的需求');
      return;
    }
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span>已选择 {selectedRequirements.length} 个需求</span>
                </div>
                <div className="flex items-center gap-2">
                  <Button 
                    variant="outline" 
            ...
            <Table>
              <TableHeader className="sticky top-0 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 z-10">
                <TableRow>
                  <TableHead className="w-12 px-2">
                    <Checkbox
                      checked={
                        ...
                        selectedRequirements.length === sortedRequirements.length
                      }
                      onCheckedChange={handleSelectAll}
                    />
                  </TableHead>
                  <TableHead className="min-w-[250px]">需求标题</TableHead>
                  ...
                  {!hiddenColumns.includes('reviewer1Status') && <TableHead>一级评审状态</TableHead>}
                  {!hiddenColumns.includes('reviewer2Status') && <TableHead>二级评审状态</TableHead>}
                  {!hiddenColumns.includes('scheduledVersion') && <TableHead>预排期版本号</TableHead>}
                </TableRow>
              </TableHeader>
              <TableBody>
                {sortedRequirements.map((requirement) => (
                  <TableRow key={requirement.id}>
                    ...
                    <TableCell>
                      ...
                      >
                        <div className="max-w-[300px] truncate">{requirement.title}</div>
                      </div>
                    </TableCell>
                      ...
                      <TableCell>
                        {requirement.reviewer1 ? (
                          <Select
                            ...
                            onValueChange={(value) => 
                              handleReviewer1StatusChange(requirement.id, value as 'pending' | 'approved' | 'rejected')
                            }
                          >
                      ...
                      <TableCell>
                        {requirement.reviewer2 ? (
                          <Select
                            ...
                            onValueChange={(value) => 
                              handleReviewer2StatusChange(requirement.id, value as 'pending' | 'approved' | 'rejected')
                            }
                          >
                            <SelectTrigger className="w-24 h-7">
                              <SelectValue />
                    ...
                    {!hiddenColumns.includes('scheduledVersion') && (
                      <TableCell>
                        {canSetVersion(requirement) ? (
                          <Select
                            value={requirement.scheduledVersion || 'unset'}
                            onValueChange={(value) => 
                              handleVersionChange(requirement.id, value === 'unset' ? 'clear' : value)
                            }
                          >
                            <SelectTrigger className="w-24 h-7">
