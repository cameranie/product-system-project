...
interface PrototypeCommentsProps {
  prototypeId: string;
  currentUser: User;
}
...
const mockComments: Comment[] = [
  {
    id: '1',
    content: '这个原型的用户交互流程很清晰，但建议在支付步骤增加更多的确认提示，提升用户安全感。',
    author: mockUsers[1],
    ...
    replies: [
      {
        id: '1-1', 
        content: '同意，特别是大额支付的时候确实需要多重确认',
        author: mockUsers[2],
        createdAt: '2024-12-16 15:45'
      }
    ]
  },
  {
    id: '2',
    content: '移动端的适配效果不错，但平板端的布局可能需要优化一下，某些按钮在平板上显示得过小。',
    author: mockUsers[2],
    createdAt: '2024-12-16 09:15',
    updatedAt: '2024-12-16 10:20',
    isEdited: true
  },
  {
    id: '3',
    content: 'Figma原型已经更新到最新版本，包含了所有的边界情况处理。可以开始技术评审了。',
    author: mockUsers[3],
    createdAt: '2024-12-16 16:20'
  }
];
...
export function PrototypeComments({ prototypeId, currentUser }: PrototypeCommentsProps) {
  ...
  const renderComment = (comment: Comment, isReply: boolean = false, parentId?: string) => (
    <div key={comment.id} className={`space-y-3 ${isReply ? 'ml-12 pt-3 border-l-2 border-muted pl-4' : ''}`}>
      ...
      <CardContent className="space-y-6">
        ...
        {sortedComments.length > 0 ? (
          ...
          <div className="text-center py-8 text-muted-foreground">
            ...
            <div className="text-sm">暂无讨论</div>
            <div className="text-xs">成为第一个发表意见的人</div>
          </div>
        )}
      </CardContent>
    </Card>
