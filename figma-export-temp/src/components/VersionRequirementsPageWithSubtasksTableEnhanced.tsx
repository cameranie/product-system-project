...
export function VersionRequirementsPageWithSubtasksTable({ onNavigate }: { onNavigate?: (page: string, context?: any) => void }) {
  ...
  const calculateDuration = (startDate: string, endDate: string): number => {
    ...
    return diffDays * 8; // 一天按8小时工作时间计算
  };
                                    ...
                                    <Table>
                                      <TableHeader>
                                        <TableRow>
                                          ...
                                          <TableHead className="text-xs">预计开始</TableHead>
                                          <TableHead className="text-xs">预计结束</TableHead>
                                          <TableHead className="text-xs">预估耗时(h)</TableHead>
                                          <TableHead className="text-xs">实际开始</TableHead>
                                          <TableHead className="text-xs">实际结束</TableHead>
                                          <TableHead className="text-xs">实际耗时(h)</TableHead>
                                          <TableHead className="text-xs">延期状态</TableHead>
                                        </TableRow>
                                      </TableHeader>
                                      <TableBody>
                                          ...
                                          >
                                            ...
                                            >
                                              {editingCell?.requirementId === requirement.id && 
                                               ...
                                               editingCell?.field === 'name' ? (
                                                ...
                                                <span className="hover:bg-accent px-2 py-1 rounded">{subtask.name}</span>
                                              )}
                                            </TableCell>
                                            ...
                                            <TableCell className="py-2">
                                              {subtask.executor ? (
                                                <DropdownMenu>
                                                  <DropdownMenuTrigger asChild>
                                                    <div className="flex items-center gap-2 cursor-pointer hover:bg-accent px-2 py-1 rounded">
                                                ...
                                                <DropdownMenu>
                                                  <DropdownMenuTrigger asChild>
                                                    <span className="text-xs text-muted-foreground cursor-pointer hover:bg-accent px-2 py-1 rounded">未分配</span>
                                                  </DropdownMenuTrigger>
                                                  <DropdownMenuContent>
                                                    {mockUsers.map((user) => (
                                                      <DropdownMenuItem 
                                            ...
                                            <TableCell className="py-2">
                                              <DropdownMenu>
                                                <DropdownMenuTrigger asChild>
                                                  <Badge 
                                                    variant="outline" 
                                                    className="text-xs bg-blue-100 text-blue-800 border-blue-200 cursor-pointer"
                                                  >
                                                    {subtask.status}
                                                  </Badge>
                                                </DropdownMenuTrigger>
                                            ...
                                            <TableCell 
                                              className="py-2 cursor-pointer"
                                              onClick={() => setEditingCell({ requirementId: requirement.id, subtaskId: subtask.id, field: 'estimatedDuration' })}
                                            >
                                              {editingCell?.requirementId === requirement.id && 
                                               editingCell?.subtaskId === subtask.id && 
                                               editingCell?.field === 'estimatedDuration' ? (
                                                <EditableCell
                                                  value={subtask.estimatedDuration || 0}
                                                  type="number"
                                                  onChange={(value) => updateSubtaskField(requirement.id, subtask.id, 'estimatedDuration', value)}
                                                  onSave={() => setEditingCell(null)}
                                                />
                                              ) : (
                                                <span className="text-xs text-muted-foreground hover:bg-accent px-2 py-1 rounded">
                                            ...
                                            <TableCell 
                                              className="py-2 cursor-pointer"
                                              onClick={() => setEditingCell({ requirementId: requirement.id, subtaskId: subtask.id, field: 'actualDuration' })}
                                            >
                                              {editingCell?.requirementId === requirement.id && 
                                               editingCell?.subtaskId === subtask.id && 
                                               editingCell?.field === 'actualDuration' ? (
                                                <EditableCell
                                                  value={subtask.actualDuration || 0}
                                                  type="number"
                                                  onChange={(value) => updateSubtaskField(requirement.id, subtask.id, 'actualDuration', value)}
                                                  onSave={() => setEditingCell(null)}
                                                />
                                              ) : (
                                                <span className="text-xs text-muted-foreground hover:bg-accent px-2 py-1 rounded">
                                            ...
                                            <TableCell className="py-2">
                                              <DropdownMenu>
                                                <DropdownMenuTrigger asChild>
                                                  <Badge 
                                                    variant="outline" 
                                                    className={`text-xs cursor-pointer ${getDelayStatusColor(subtask.delayStatus)}`}
                                                  >
                                                    {subtask.delayStatus}
                                                  </Badge>
                                                </DropdownMenuTrigger>
                                                <DropdownMenuContent>
                                                  {delayStatusOptions.map((status) => (
                                                    <DropdownMenuItem 
                                                      key={status}
                                                      onClick={() => updateSubtaskField(requirement.id, subtask.id, 'delayStatus', status)}
                                                    >
                                                      {status}
                                                    </DropdownMenuItem>
                                                  ))}
