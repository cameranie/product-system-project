...
const mockComments: Comment[] = [
  {
    id: '1',
    content: '这个需求的优先级建议调整为高，因为用户反馈较多，影响用户体验。建议在下个版本中优先实现。',
    author: mockUsers[1],
  ...
  {
    id: '2',
    content: '需要考虑兼容性问题，特别是在移动端的表现。建议添加responsive设计。',
    author: mockUsers[2],
...
export function RequirementComments({ requirementId, currentUser }: RequirementCommentsProps) {
      ...
      if (!isReply && comment.id === commentId) {
        ...
        return {
          ...
          replies: comment.replies?.map(reply => 
            reply.id === commentId ? updateLikes(reply) : reply
          )
        };
      }
      return comment;
      ...
      <CardContent className="space-y-6">
        ...
        {sortedComments.length > 0 ? (
          ...
          <div className="text-center py-8 text-muted-foreground">
            ...
            <div className="text-xs">成为第一个发表评论的人</div>
          </div>
        )}
      </CardContent>
    </Card>
