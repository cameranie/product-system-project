...
interface Prototype {
  ...
  prototypeType?: '交互原型' | '低保真原型' | '高保真原型' | '流程图' | 'App原型' | 'Web原型';
}
    ...
    return filters.every(filter => {
      ...
      switch (filter.column) {
        ...
        case 'platform':
          ...
          break;
        case 'reviewStatus':
          // 这里需要根据评审状态组件的逻辑来获取状态
          fieldValue = prototype.reviewStatus || 'pending';
          break;
        case 'reviewer1Status':
          fieldValue = prototype.reviewer1Status ? reviewerStatusLabels[prototype.reviewer1Status].label : '';
          break;
        case 'reviewer2Status':
          fieldValue = prototype.reviewer2Status ? reviewerStatusLabels[prototype.reviewer2Status].label : '';
          break;
        case 'createdAt':
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-muted-foreground mt-1">原型和交互设计管理</p>
            </div>
          </div>
                    ...
                    {sortedPrototypes.map((prototype) => (
                      <TableRow key={prototype.id} className="cursor-pointer hover:bg-muted/50" onClick={() => handleViewPrototype(prototype)}>
                        ...
                        {!hiddenColumns.includes('reviewStatus') && (
                          <TableCell>
                            <ReviewStatusBadge item={prototype} />
                          </TableCell>
                        )}
                        ...
                        {!hiddenColumns.includes('reviewer1Status') && (
                          <TableCell>
                            <Reviewer1Component 
                              item={prototype}
                              onStatusChange={(value) => handleReviewer1StatusChange(prototype.id, value)}
                            />
                          </TableCell>
                        )}
                        ...
                        {!hiddenColumns.includes('reviewer2Status') && (
                          <TableCell>
                            <Reviewer2Component 
                              item={prototype}
                              onStatusChange={(value) => handleReviewer2StatusChange(prototype.id, value)}
                            />
                          </TableCell>
                        )}
