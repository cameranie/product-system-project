...
const mockBugs: Bug[] = [
  ...
  // 为需求1添加更多bugs
  {
    ...
    project: mockProjects[0],
    requirement: { id: '1', title: '用户注册流程优化' },
    reproductionSteps: '1. 进入注册页面\\n2. 输入企业邮箱地址\\n3. 点击注册按钮',
    expectedResult: '能够正常注册成功',
    actualResult: '提示邮箱格式不正确',
    environment: 'Chrome 120.0, Windows 11, 分辨率1920x1080',
  ...
  },
  {
    ...
    project: mockProjects[0],
    requirement: { id: '1', title: '用户注册流程优化' },
    reproductionSteps: '1. 点击微信登录\\n2. 在微信中授权\\n3. 返回应用',
    expectedResult: '自动完成登录并进入主页',
    actualResult: '页面显示回调错误信息',
    environment: 'iOS 16.0, 微信 8.0.32',
  ...
  // 为需求2添加更多bugs
  {
    ...
    project: mockProjects[1],
    requirement: { id: '2', title: '支付功能集成' },
    reproductionSteps: '1. 选择微信支付\\n2. 完成支付\\n3. 返回应用查看订单状态',
    expectedResult: '订单状态更新为已支付',
    actualResult: '订单状态仍为待支付',
    environment: 'iOS 16.0, 微信 8.0.32',
  ...
  // 为需求4添加bugs
  {
    ...
    project: mockProjects[0],
    requirement: { id: '4', title: 'K线图实时更新优化' },
    reproductionSteps: '1. 打开K线图页面\\n2. 观察实时价格变化\\n3. 对比其他平台数据',
    expectedResult: '数据更新延迟不超过1秒',
    actualResult: '数据更新延迟3-5秒',
    environment: 'Chrome 120.0, Windows 11',
    createdAt: '2024-01-20 09:30',
    updatedAt: '2024-01-22 16:15',
    tags: ['K线', '实时数据', '性能']
  },
  {
    id: '8',
    ...
    project: mockProjects[0],
    requirement: { id: '4', title: 'K线图实时更新优化' },
    reproductionSteps: '1. 选择MACD指标\\n2. 对比标准计算结果\\n3. 验证数值准确性',
    expectedResult: '技术指标计算准确无误',
    actualResult: '计算结果存在偏差',
    environment: 'Chrome 120.0, macOS Monterey',
...
export function BugsPageUpdated({ context, onNavigate }: BugsPageProps = {}) {
  ...
  const filteredBugs = bugs.filter(bug => {
    ...
    // 如果从需求详情页跳转过来，只显示该需求相关的bugs
    const matchesRequirement = !context?.filterRequirementId || 
                               bug.requirement?.id === context.filterRequirementId;
    
    return matchesSearch && matchesStatus && matchesPriority && matchesProject && matchesRequirement;
  });
