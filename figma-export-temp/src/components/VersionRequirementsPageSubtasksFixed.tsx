...
interface Subtask {
  ...
  estimatedDuration?: number; // ��估耗时（小时）
...
export function VersionRequirementsPageWithSubtasksTable({ onNavigate }: { onNavigate?: (page: string, context?: any) => void }) {
  ...
  const ContextMenu: React.FC<{
    ...
    onInsertBelow: () => void;
    onDeleteTask: () => void;
  }> = ({ x, y, onClose, onCopyTask, onInsertAbove, onInsertBelow, onDeleteTask }) => {
    ...
    return (
      <div
        ref={menuRef}
        className="fixed bg-background border rounded-md shadow-lg py-1 z-50 min-w-[140px]"
        style={{ left: x, top: y }}
      >
        <button
    ...
    <div className="h-screen flex flex-col">
                                  ...
                                  <div className="p-4">
                                    ...
                                    <div className="w-full overflow-hidden">
                                      <style jsx>{`
                                        ...
                                        .executor-cell { width: 100px; }
                                        .department-cell { width: 80px; }
                                        .status-cell { width: 80px; }
                                        .date-cell { width: 140px; }
                                        .duration-cell { width: 60px; text-align: center; }
                                        .delay-cell { width: 60px; text-align: center; }
                                      `}</style>
                                      
                                      <table className="subtask-table">
                                        <thead>
                                          <tr>
                                            <th className="task-name-cell">任务名称</th>
                                            <th className="executor-cell">执行人</th>
                                            <th className="department-cell">部门</th>
                                            <th className="status-cell">状态</th>
                                            <th className="date-cell">预计开始</th>
                                            <th className="date-cell">预计结束</th>
                                            <th className="duration-cell">预估耗时(h)</th>
                                            <th className="date-cell">实际开始</th>
                                            <th className="date-cell">实际结束</th>
                                            <th className="duration-cell">实际耗时(h)</th>
                                            <th className="delay-cell">延期状态</th>
                                          </tr>
                                        </thead>
                                        <tbody>
                                            ...
                                            >
                                                ...
                                                {editingCell?.requirementId === requirement.id && 
                                                 ...
                                                 editingCell?.field === 'name' ? (
                                                  <input
                                                    ...
                                                    className="w-full h-8 text-xs px-2 py-1 border border-input rounded-md bg-background"
                                                    autoFocus
                                                  />
                                                ) : (
                                                  <div 
                                              ...
                                              <td className="executor-cell">
                                                {subtask.executor ? (
                                                  <DropdownMenu>
                                                    <DropdownMenuTrigger asChild>
                                                      <div className="flex items-center gap-1 cursor-pointer hover:bg-accent px-2 py-1 rounded">
                                                        <Avatar className="h-4 w-4 flex-shrink-0">
                                                          <AvatarFallback className="text-xs">{subtask.executor.name[0]}</AvatarFallback>
                                                        </Avatar>
                                                        <span className="truncate text-xs">{subtask.executor.name}</span>
                                                      </div>
                                                    </DropdownMenuTrigger>
                                                    <DropdownMenuContent>
                                                  ...
                                                  <DropdownMenu>
                                                    <DropdownMenuTrigger asChild>
                                                      <span className="text-muted-foreground cursor-pointer hover:bg-accent px-2 py-1 rounded block text-center text-xs">未分配</span>
                                                    </DropdownMenuTrigger>
                                                    <DropdownMenuContent>
                                                      {mockUsers.map((user) => (
                                                        <DropdownMenuItem 
                                              ...
                                              <td className="status-cell">
                                                <DropdownMenu>
                                                  <DropdownMenuTrigger asChild>
                                                    <Badge 
                                                      variant="outline" 
                                                      className={`text-xs cursor-pointer w-full justify-center ${getStatusColor(subtask.status)}`}
                                                    >
                                              ...
                                              <td className="date-cell">
                                                {editingCell?.requirementId === requirement.id && 
                                                 ...
                                                 editingCell?.field === 'estimatedStartDate' ? (
                                                  <input
                                                    ...
                                                    onChange={(e) => {
                                                      ...
                                                      if (value) {
                                                        ...
                                                        updateSubtaskField(requirement.id, subtask.id, 'estimatedStartDate', date.toISOString());
                                                      } else {
                                                        updateSubtaskField(requirement.id, subtask.id, 'estimatedStartDate', '');
                                                      }
                                                    }}
                                                    onBlur={() => setEditingCell(null)}
                                                    onKeyDown={(e) => {
                                                  ...
                                                  <div 
                                                    className="cursor-pointer hover:bg-accent px-2 py-1 rounded text-muted-foreground text-xs"
                                                 ...
                                                 editingCell?.field === 'estimatedEndDate' ? (
                                                  <input
                                                    ...
                                                    onChange={(e) => {
                                                      ...
                                                      if (value) {
                                                        const date = new Date(value);
                                                        updateSubtaskField(requirement.id, subtask.id, 'estimatedEndDate', date.toISOString());
                                                      } else {
                                                        updateSubtaskField(requirement.id, subtask.id, 'estimatedEndDate', '');
                                                      }
                                                    }}
                                                    onBlur={() => setEditingCell(null)}
                                                    onKeyDown={(e) => {
                                                 ...
                                                 editingCell?.field === 'actualStartDate' ? (
                                                  <input
                                                    ...
                                                    onChange={(e) => {
                                                      ...
                                                      if (value) {
                                                        const date = new Date(value);
                                                        updateSubtaskField(requirement.id, subtask.id, 'actualStartDate', date.toISOString());
                                                      } else {
                                                        updateSubtaskField(requirement.id, subtask.id, 'actualStartDate', '');
                                                      }
                                                    }}
                                                    onBlur={() => setEditingCell(null)}
                                                    onKeyDown={(e) => {
                                                 ...
                                                 editingCell?.field === 'actualEndDate' ? (
                                                  <input
                                                    ...
                                                    onChange={(e) => {
                                                      ...
                                                      if (value) {
                                                        const date = new Date(value);
                                                        updateSubtaskField(requirement.id, subtask.id, 'actualEndDate', date.toISOString());
                                                      } else {
                                                        updateSubtaskField(requirement.id, subtask.id, 'actualEndDate', '');
                                                      }
                                                    }}
                                                    onBlur={() => setEditingCell(null)}
                                                    onKeyDown={(e) => {
      ...
      {contextMenu && (
        <ContextMenu
          ...
          onInsertAbove={() => handleInsertSubtask(contextMenu.requirementId, contextMenu.subtaskId, 'above')}
          onInsertBelow={() => handleInsertSubtask(contextMenu.requirementId, contextMenu.subtaskId, 'below')}
